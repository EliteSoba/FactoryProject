	...
	public class LanePreferencesPanel extends JPanel implements ActionListener{
		...
		public void actionPerformed(ActionEvent ae) {
			...
			if(ae.getSource() == laneOn){ // turn lane one
				...
				String set = "fpm lm set lanepower on " + (lanenum-1);
				try {
					factoryProductionManager.sendCommand(set);
				} catch (Exception e) {
				...
				int lanenum = (Integer)laneSelect.getSelectedItem();
				String set = "fpm lm set lanepower off " + (lanenum-1);
				try {
					factoryProductionManager.sendCommand(set);
				} catch (Exception e) {
				...
				String set = "fpm lm set feederpower on " + (feedernum-1);
				try {
					factoryProductionManager.sendCommand(set);
				} catch (Exception e) {
				...
				int feedernum = (Integer)feederSelect.getSelectedItem();
				String set = "fpm lm set feederpower off " + (feedernum-1);
				try {
					factoryProductionManager.sendCommand(set);
				} catch (Exception e) {
				...
				if(cb == laneSelect){
					System.out.println(factoryProductionManager.getLaneSpeed((Integer)cb.getSelectedItem()));
					laneSpeed.setValue(factoryProductionManager.getLaneSpeed((Integer)cb.getSelectedItem()));
					laneAmplitude.setValue(factoryProductionManager.getLaneAmplitude((Integer)cb.getSelectedItem()));
				}
			}
			...
			public void stateChanged(ChangeEvent ce) {
				...
				if(source == laneSpeed){
					if (!source.getValueIsAdjusting()) {
						...
						int lanenum = (Integer)laneSelect.getSelectedItem();
						String set = "fpm lm set lanespeed " + (lanenum-1) + " " + speed;
						try {
							factoryProductionManager.sendCommand(set);
						} catch (Exception e) {
					...
					if (!source.getValueIsAdjusting()) {
						...
						int lanenum = (Integer)laneSelect.getSelectedItem();
						String set = "fpm lm set laneamplitude " + (lanenum-1) + " " + amplitude;
						try {
							factoryProductionManager.sendCommand(set);
						} catch (Exception e) {
