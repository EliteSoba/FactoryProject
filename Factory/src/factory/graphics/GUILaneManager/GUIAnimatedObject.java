...
class GUIAnimatedObject
{
	...
	protected ArrayList<GUIGraphicItem> items;			// inventory of items
	
	// Constructors
	public GUIAnimatedObject()
	{
		
	}
	public GUIAnimatedObject(int init_x, int init_y, int init_theta, int init_dx, int init_dy, int init_dtheta, int init_imageWidth, int init_imageHeight, String init_imagePath)
	{
		items = new ArrayList<GUIGraphicItem>();
		x = init_x;
		y = init_y;
		theta = init_theta;
		dx = init_dx;
	...
	public void addItem(GUIGraphicItem newItem)
	{
		items.add(newItem);
	}
	...
	public GUIGraphicItem popItem()
	{
		GUIGraphicItem lastItem = items.get(items.size()-1);		// get last item
	...
	public GUIGraphicItem getItemAt(int i)
	{
		return items.get(i);
	}
	// Update functions
