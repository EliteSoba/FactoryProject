...
public class FCSTests extends TestCase{
	...
	public void testFinishOrderAndTakeNewOrder(){

		//initialize all components for this test
		KitProductionState stateFinished = KitProductionState.FINISHED;
		KitProductionState statePending = KitProductionState.PENDING;
		KitProductionState stateProducing = KitProductionState.PRODUCING;
		KitConfig kit1 = new KitConfig();
		KitConfig kit2 = new KitConfig();
		MockGantry gantry = new MockGantry("gantry");
		MockPartsRobot partsRobot = new MockPartsRobot("name");
		FCSAgent fcs = new FCSAgent(gantry, partsRobot, null);
		...
		//message from panel to make an order type
		fcs.msgProduceKit(2, "typeA");

		//test to see if FCS got the order
		assertEquals("FCS order queue should be size one, but instead is: " + fcs.orders.size(),
				1, fcs.orders.size());

		//check the FCS state
		assertEquals("FCS state should be PENDING, but instead is: " + fcs.state, statePending, fcs.state);

		//call scheduler
		fcs.pickAndExecuteAnAction();

		//check the FCS state
		assertEquals("FCS state should be PRODUCING, but instead is: " + fcs.state, stateProducing, fcs.state);

		//tests to see if the parts robot log got the message from the FCS

		assertTrue("Parts Robot should have gotten a message to make a kit from the FCS.", partsRobot.log
				.getLastLoggedEvent().getMessage().contains("msgMakeKit"));
		...
		//FCS should still be ordering kit
		assertEquals("FCS order queue should be size one, but instead is: " + fcs.orders.size(),
				1, fcs.orders.size());
		...
		//FCS should still not be deleted at this point
		assertEquals("FCS order queue should be size 1, but instead is: " + fcs.orders.size(),
				1, fcs.orders.size());

		//check the FCS state
		assertEquals("FCS state should be FINISHED, but instead is: " + fcs.state, stateFinished, fcs.state);
		...
		assertEquals("FCS order queue should be size 0, but instead is: " + fcs.orders.size(),
				0, fcs.orders.size());

		//make sure parts robot gets message when orders is empty
		assertTrue("Parts Robot should have gotten a message that there are no more orders from the FCS.", partsRobot.log
				.getLastLoggedEvent().getMessage().contains("msgNoMoreOrders"));

		//message from panel to make an order type
		fcs.msgProduceKit(1, "typeB");
		...
		//order size should be 0
		assertEquals("FCS order queue should be size 0, but instead is: " + fcs.orders.size(),
				0, fcs.orders.size());
		
		//make sure parts robot old message isn't last logged
		assertFalse("Last logged event shouldn't be msgMakeKit.", partsRobot.log
				.getLastLoggedEvent().getMessage().contains("msgMakeKit"));
		...
		//check the FCS state
		assertEquals("FCS state should still be PENDING, but instead is: " + fcs.state, statePending, fcs.state);
