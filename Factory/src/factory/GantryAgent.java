...
public class GantryAgent extends Agent implements Gantry {
	...
	private void DoPickupPurgeBin(MyBin b) {
		...
		if(!unitTesting){
			server.command(this,"ga gm cmd pickuppurgebin " + b.fdr.getFeederNumber());
			try{
				animation.acquire();
			}
	...
	private void DoGoGetNewBin(MyBin b) {
		...
		if(!unitTesting){
			server.command(this,"ga gm cmd getnewbin " + b.pt.name);
			try{
				animation.acquire();
			}
			catch (InterruptedException e){
	...
	private void DoBringNewBin(MyBin b) {
		...
		if(!unitTesting){
			server.command(this,"ga gm cmd bringbin " + b.fdr.getFeederNumber());
			try{
				animation.acquire();
			}
			catch (InterruptedException e){
